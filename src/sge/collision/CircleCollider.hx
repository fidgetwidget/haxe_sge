...
class CircleCollider extends Collider
{
	...
	public var circle(get, set)				:Circle;
	...
	public override function collideBox( b:BoxCollider, cdata:CollisionData = null ) :Bool {
		return CollisionMath.circleBoxCollision( this, b.get_bounds(), cdata );
	}
	...
	public override function contains( x:Float, y:Float ) :Bool {
		...
		if (_e != null) {
			return CollisionMath.distanceBetween( _e.x + _circle.x, _e.y + _circle.y, x, y ) <= _circle.radius;
		} else {
			return CollisionMath.distanceBetween( _circle.x, _circle.y, x, y ) <= _circle.radius;
		}		
	}
